...
router.post("/preview", async (req, res) => {
  try {
    ...
    if (!accuracy || accuracy > 100) {
      return res.status(400).json({
        message: "Location accuracy must be ≤100m",
      });
    }
    ...
    if (!config) {
      return res.status(500).json({
        message: "Clinic config not found",
      });
    }
    ...
    if (distanceData.finalDistance > MAX_HOME_VISIT_KM) {
      return res.status(400).json({
        message: `Home visit available within ${MAX_HOME_VISIT_KM} km only`,
      });
    }

    const totalAmount = calculateHomeVisitPrice(
      distanceData.finalDistance
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!location || !location.lat || !location.lng) {
        return res.status(400).json({
          message: "GPS location is required for home visit",
        });
      }
      ...
      if (!accuracy || accuracy > 100) {
        return res.status(400).json({
          message:
            "Location accuracy is low. Please turn ON GPS, go outdoors, and try again.",
        });
      }
      ...
      const amount = calculateHomeVisitPrice(
        distanceData.finalDistance
      );

      if (!amount || amount <= 0) {
        return res.status(400).json({
          message: "Invalid home visit amount",
        });
      }
      ...
      const order = await razorpay.orders.create({
        ...
        notes: {
          ...
          baseDistanceKm:
            distanceData.baseDistance,

          accuracyKm:
            distanceData.accuracyKm,
      ...
      res.status(400).json({
        message:
          err.message ||
          "Home visit payment initiation failed",
      });
    }
  }
);
