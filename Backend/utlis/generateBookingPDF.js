...
const generateBookingPDF = (booking) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(
        "📋 PDF Generation Started for Booking:",
        booking._id
      );
      ...
      console.log("🔍 Booking Analysis:", {
        ...
        isMongooseObject:
          booking.slotId instanceof mongoose.Types.ObjectId,
        hasStartTime:
          booking.slotId?.startTime !== undefined,
        hasEndTime:
          booking.slotId?.endTime !== undefined,
      ...
      if (booking.visitType === "CLINIC") {
        ...
        if (slotData) {
          // ObjectId / string ID
          if (
            slotData instanceof mongoose.Types.ObjectId ||
            (typeof slotData === "object" &&
              slotData._bsontype === "ObjectId") ||
            (typeof slotData === "string" &&
              mongoose.Types.ObjectId.isValid(slotData))
          ) {
            console.log(
              "🔍 Slot is ObjectId, fetching slot data..."
            );

            try {
              const fetchedSlot = await Slot.findById(
                slotData
              );

              if (fetchedSlot && fetchedSlot.startTime) {
                console.log(
                  "✅ Successfully fetched slot data"
                );
                ...
                console.log(
                  "⚠️ Could not fetch slot data or slot has no startTime"
                );
              }
            } catch (fetchError) {
              console.error(
        ...
        if (
          ...
          // ✅ INDIA TIMEZONE
          dateStr = formatDateIST(start);

          timeStr = `${formatTimeIST(start)} – ${formatTimeIST(
            end
          )}`;

          console.log(
            "✅ Using SLOT time for CLINIC visit:"
          );
          ...
          console.log(
            "   Original Slot End:",
            slotData.endTime ||
              "Not specified (using 30min default)"
          );
        } else {
          console.log(
            "⚠️ No valid slot data available, using createdAt"
          );
      ...
      // ==================================================
        ...
        if (
          booking.visitTime &&
          typeof booking.visitTime === "string"
        ) {
          try {
            const [startISO, endISO] =
              booking.visitTime.split("|");
            ...
            timeStr = `${formatTimeIST(
              start
            )} – ${formatTimeIST(end)}`;

            console.log(
              "✅ Using visitTime for HOME visit:",
              booking.visitTime
            );
          } catch (error) {
            console.log(
              "⚠️ Failed to parse visitTime, using createdAt"
            );
          ...
          console.log(
            "⚠️ HOME booking has no visitTime, using createdAt"
          );
      ...
      doc
        ...
        .fillColor("#CBD5E1")
        .text(
          "Consultant Physiotherapist | BPT (Kol), FIMT, NABH"
        )
        .text(
          "Reg No: 20013000306 | Ex-Intern, Chanchal Super Speciality Hospital"
        );
      ...
      if (booking.visitType === "HOME") {
        ...
        doc
          .moveDown(0.3)
          .text(
            `Base Charge (4 km): ₹${base}`
          )
          .text(
            `Extra Charge (${extraKm} km × ₹20): ₹${extraCharge}`
          );
      ...
      doc
        ...
        .fillColor("#F8FAFC")
        .text(
          "Kaliachak Advance Physiotherapy Clinic",
          {
            align: "center",
          }
        );
